...
def getWhere(request,qs):
    ...
    if request.GET.get("gonghao"):
        qs = qs.filter(gonghao__contains=request.GET.get("gonghao"))
    if request.GET.get("xingming"):
        qs = qs.filter(xingming__contains=request.GET.get("xingming"))
    if request.GET.get("xingbie"):
        qs = qs.filter(xingbie=request.GET.get("xingbie"))
    if request.GET.get("suodaibanji"):
        qs = qs.filter(suodaibanji=request.GET.get("suodaibanji"))
...
def adminlist(request):
    ...
    # 渲染模板文件：templates/jiaoshi/list.html
    return render(request , "jiaoshi/list.html" , locals() )
...
def adminadd(request):
    ...
    # 渲染模板文件：templates/jiaoshi/add.html
    return render(request , "jiaoshi/add.html",locals())
...
def adminupdt(request):
    ...
    # 渲染模板文件：templates/jiaoshi/updt.html
    return render(request, "jiaoshi/updt.html" , locals())
...
def adminupdtself(request):
    ...
    # 设置为更新用户资料
    updtself = 1
    # 渲染模板文件：templates/jiaoshi/updtself.html
    return render(request, "jiaoshi/updtself.html" , locals())
...
def admindetail(request):
    ...
    # 渲染模板文件：templates/jiaoshi/detail.html
    return render(request , "jiaoshi/detail.html" , locals())
...
def insert(request):
    ...
    data = {
        ...
        'dianziyouxiang': utils.param('dianziyouxiang',''),
        'suodaibanji': utils.param('suodaibanji',utils.param('suodaibanji',None)),
    ...
    # 新建Jiaoshi 对象，用JiaoshiModel 对象进行对数据进行插入
    model = Jiaoshi(**data)
    # 执行插入数据
...
def update(request):
    ...
    data = {
        ...
        'dianziyouxiang': utils.param('dianziyouxiang',old.dianziyouxiang),
        'suodaibanji': utils.param('suodaibanji',utils.param('suodaibanji',old.suodaibanji)),
        'jianjie': utils.param('jianjie',old.jianjie),

    }


    # 新建Jiaoshi 对象，用JiaoshiModel 对象进行对数据进行更新
    model = Jiaoshi(**data)
    # 执行更新操作
    ...
    if updtself == '' or updtself == '0':
        ...
        referer = "/jiaoshi/admin/updtself/"
